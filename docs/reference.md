...
| `/planboard:handoff` | Write or refresh the codex handoff — a marked planboard block in `AGENTS.md` so a cooperative codex can run the plan/execute/results loop by reading the plugin's references and scripts. Review and signing stay in a Claude session. |

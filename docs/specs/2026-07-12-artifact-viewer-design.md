...
- Non-viewable files keep an anchor, policy per URL kind (derived from `url.startsWith("data:")`, no prop threading):
  - Live (`/artifact/...`) URLs: the `download` attribute is dropped **only** for types safe to open in a tab (the text whitelist, raster images, `.pdf`); those get `target="_blank" rel="noopener"`. Active or unknown types (`.html`, `.xml`, `.xlsx`, anything else) **keep** the `download` attribute — never an inline navigation path for active content. `.svg` also keeps it: the server serves svg inline so `<img>` figures render, but sandboxed (CSP), and top-level navigation to a sandboxed svg stalls Chrome's load events (observed in the browser smoke) — so svg links download rather than navigate.
...
- 2026-07-13 — post-smoke: `.svg` removed from the anchor inline-safe set (top-level navigation to a CSP-sandboxed svg stalled Chrome's load events in the Playwright smoke; figures render via `<img>` regardless, which the smoke verified). Server policy unchanged.

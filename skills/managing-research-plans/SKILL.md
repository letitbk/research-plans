...
- **Versions are immutable — and mechanically enforced.** `v1.md, v2.md, ...` are never overwritten or edited; the plugin's sign-off gate (a PreToolUse hook) blocks edits to signed versions and opens a browser approval for every new version write. Approval happens on the board, not in the terminal — primarily on the persistent board's review room (draft → scorecard → Approve mints a ticket; the routing writes the signed file), with the blocking modal gate as the fallback for direct `vN.md` writes. If the gate denies, read `plans/.board-feedback.md`, revise, and write again. If it times out, the proposal is saved as the component's `.draft-vN.md` — recovery is the researcher's Approve on the persistent board (mints the same durable ticket), never the bypass. Batch sign-off (`board.py --gate-batch`) is the `/adopt` bulk flow only and refuses fewer than 2 pending drafts (`--allow-single` is the explicit exception). Headless/CI sessions set `RESEARCH_PLANS_NO_GATE=1` (the bypass leaves a stderr trace). Deviations are recorded, never hidden — especially the improvised ones.
